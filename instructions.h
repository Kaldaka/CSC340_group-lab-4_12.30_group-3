/*** Here are the instructions for the lab if you want to work offline ***/
...
/*
Project description:
	In this project, you are going to analyze one of Steve Jobs' famous commencement speech 
	delivered in 2005 at Stanford University. You can download this speech here to test your program 
	locally before submitting to zyBook. Specifically, you are going to process this file to identify 
	all the co-occurring word-pairs and their frequencies. Two words are said to co-occur if they 
	appear in the same sentence. We will refer to a pair of co-occurring words as word-pairs. 
...
Header file: fileIOs_wordPairs.h
	...
	Given a paragraph of multiple sentences, the following punctuations will be used to 
	split this paragraph into individual sentences:
	...
	this function identifies all the unique word-pairs and each word-pair's frequency. 
	...
	Note that	
		...
		Order does not matter between two words in a word-pair. 
			For example, the word-pair <college, that> is the same as <that, college>. 
			You are recommended to arrange the two words in lexicographical order before inserting the pair into the map.
